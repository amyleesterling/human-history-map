...
// A shared narrative has one evidence packet, while each map destination still
// needs its own identity, chronology and geometry assessment.
const supportsCard=(record,id,card)=>record.id===id || (Array.isArray(card.appliesTo)&&card.appliesTo.includes(id)&&record.id===card.id);
const themes={
