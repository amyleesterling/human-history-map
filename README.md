...
```
index.html, civ.html     the two pages, in the black sci-fi skin
atlas.html, atlas-civ.html   the same two as ink on paper, generated by
                         scripts/build-atlas.mjs (never edited by hand)
site.css                 everything both pages share, and the sci-fi paint
atlas.css                the atlas skin's paint, loaded only by the atlas pages
...
scripts/build-atlas.mjs  regenerates the atlas pages; --check is run by validate
